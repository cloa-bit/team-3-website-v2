...
Police brutality has long been an issue in the United States. The recent killing of George Floyd has again cast a spotlight on policing practices in America. We decided to study data on the use of force by police in Minneapolis. Breaking it down by race over the entrie data set we have the following.
...
We can also break it down by year.
...
ggplot(data = total_yr, mapping = aes(x=Year)) +
  ...
  ggtitle("Case numbers from 2008 and 2018", "Value on red (blue) dots represent percentage (%) of male (black male).")
```

And we can also break it down by age.
...
Something imporant to keep in mind is that **only 19.4% of the population of Minneapolis is black** ^["U.S. Census website". United States Census Bureau. July 1, 2019.]. So it's obvious that a African Americans are disproportionately represented in the data. The following graphic breaks down the cases by the problem that instigated the police to use force.
```{r, echo=FALSE, message=FALSE}
police %>%
  group_by(Problem) %>%
  summarize(n=n()) %>%
...
So the majority of cases take place in the Downtown West neighborhood of Minneapolis, the heart of the city.


Focusing on the two most represented racial groups in the most represented neighborhood, we break it down by force type.
...
police %>%
  ...
  ggplot() +
    ...
    theme(axis.text.x = element_text(angle = 45, vjust = .5)) +
    labs(x = "Race", y = "Cases", color = "Force Type", fill = "Force Type")
```
...
police %>%
  ...
  ggplot() +
    ...
    theme(axis.text.x = element_text(angle = 45, vjust = .5)) +
    labs(x = "Race", y = "Cases", color = "Force Type Action", fill = "Force Type Action")
```
